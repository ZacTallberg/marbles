...
public class cameraFollowActive : MonoBehaviour {
	...
	void Update () {
		if (activeObj == null)
			activeObj = transform.Find ("directionObj");
		
		//Vector3 marblePosition = MarbleGravityObject.ActiveMarble.transform.position;
		cameraToMarble = cameraObject.transform.position - averageMarblePos.transform.position;
		distanceBetween = cameraToMarble.magnitude;
		Vector3 marbleToPlanet = planet.transform.position - averageMarblePos.transform.position;
